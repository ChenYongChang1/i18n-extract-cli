...
import getLang, { saveLocale } from "./utils/getLang";
...
import { genreObjectDeep } from "./utils/flatObjectDeep";
import { genreLocalExportJs } from "./utils/util";

export { translateLaguage } from "./utils/translateMoreLanguage";
...
function saveAllLocaleData(localePath: string, localeFileType: string) {
  const keyMap = Collector.getKeyMap();
  const resultKeyMap = genreObjectDeep(keyMap);
  const filesPath = Object.keys(resultKeyMap);
  filesPath.forEach((filePath) => {
    saveLocale(
      [localePath, "zh-cn", filePath + "." + localeFileType].join("/"),
      resultKeyMap[filePath] as StringObject
    );
  });
}
...
export default async function (options: CommandOptions) {
  ...
  if (!fs.existsSync(primaryLangPath)) {
    ...
    fs.mkdirSync(primaryLangPath, { recursive: true });
  }
  ...
  if (!skipExtract) {
    ...
    genreLocalExportJs(localePath, "zh-cn");
    bar.stop();
